...
# 正規確率紙に累積相対度数をプロットする
PnormL <- function(Variable,Xlabel="Observed Value",MainTitle="Normal Probability Paper",                            # データベクトル
                  color="gray",names=c("1","2","3"),pchs=c(0,1,2))          # データベクトル
{
  Lnum<-length(Variable) #number of label elements
  x<-NULL
  ...
  plot(c(xmin, xmax), qnorm(c(probs[1], probs[17])), type="n", yaxt="n",
       xlab=Xlabel, ylab="Cumulative Percent",
       main=MainTitle)
...
#  points(x, qnorm(y))
  ...
  for(i in 1:Lnum){
    ...
    x <- x[!is.na(x)]                            # 欠損値を除く
    n <- length(x)                                       # 有効データ数
    y <- qnorm(1:n/(n+1))                         # 正規分布における累積密度の位置(ミーンランク)
    x <- sort(x)                                 # 昇順に並べ替える
    points(x, y,pch=pchs[i])                                #データ点を描く
  }
  legend("topleft",names,pch=pchs)
